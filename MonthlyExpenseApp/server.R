...
shinyServer(function(input, output) {
  ...
  Details_this_year<-reactive({datatable(ddply(Itemized_Monthly_Expense[Itemized_Monthly_Expense$Full_Date>=input$daterange[1]&Itemized_Monthly_Expense$Full_Date<=input$daterange[2]&Itemized_Monthly_Expense$Category%in%unlist(Past_Categorywise_Expenses_Same_Period())[input$past_Categorywise_Expenses_Same_Period_rows_selected],],"Item",summarize,Amount=sum(Amount))[order(-ddply(Itemized_Monthly_Expense[Itemized_Monthly_Expense$Full_Date>=input$daterange[1]&Itemized_Monthly_Expense$Full_Date<=input$daterange[2]&Itemized_Monthly_Expense$Category%in%unlist(Past_Categorywise_Expenses_Same_Period())[input$past_Categorywise_Expenses_Same_Period_rows_selected],],"Item",summarize,Amount=sum(Amount))[,'Amount']),],caption="This year", selection='none',options = list(searching = FALSE,paging = FALSE),rownames=FALSE)%>%formatRound(c('Amount'),digits=2)%>%formatStyle(c(1,2),`font-size`='10px')})
  Details_last_year<-reactive({datatable(ddply(Itemized_Monthly_Expense[Itemized_Monthly_Expense$Full_Date>=Last_Year_Start_Date()&Itemized_Monthly_Expense$Full_Date<=Last_Year_End_Date()&Itemized_Monthly_Expense$Category%in%unlist(Past_Categorywise_Expenses_Same_Period())[input$past_Categorywise_Expenses_Same_Period_rows_selected],],"Item",summarize,Amount=sum(Amount))[order(-ddply(Itemized_Monthly_Expense[Itemized_Monthly_Expense$Full_Date>=Last_Year_Start_Date()&Itemized_Monthly_Expense$Full_Date<=Last_Year_End_Date()&Itemized_Monthly_Expense$Category%in%unlist(Past_Categorywise_Expenses_Same_Period())[input$past_Categorywise_Expenses_Same_Period_rows_selected],],"Item",summarize,Amount=sum(Amount))[,'Amount']),],caption="Last year", selection='none',options = list(searching = FALSE,paging = FALSE),rownames=FALSE)%>%formatRound(c('Amount'),digits=2)%>%formatStyle(c(1,2),`font-size`='10px')})
